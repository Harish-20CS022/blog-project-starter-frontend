...
const API_URL = "https://blog-project-starter-backend.onrender.com/api/blogs";
...
function Blogs() {
    ...
    useEffect(() => {
        ...
        if(user){
            if(user.uid === "Lnh79UMQZ0PaQvSOMaRnrn4zuv83"){
                setAdmin(true)
            }else{
                setAdmin(false)
            }
    ...
    const handleNewBlogSubmit = (event) => {
        ...
        const likes = 0
        axios.post(API_URL, { newTitle, date, newContent, likes }).then((res) => {
            console.log(res.data)
    ...
    return (
        <div className="blog-section py-14">
            ...
            <div className="blogs-container grid grid-cols-1 md:grid-cols-2 gap-6 container mx-auto px-4 mb-6">
                {blogs.map((blog) => (
